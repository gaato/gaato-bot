...
async def opt_out(ctx: discord.ApplicationContext):
    ...
    if ctx.author.id in opt_out_users:
        await ctx.respond('Your message content is already off-track. To use other commands, please use the /opt-in command.', ephemeral=True)
    else:
        opt_out_users.append(ctx.author.id)
        with open(BASE_DIR / 'data' / 'opt-out-users.txt', 'w') as f:
            for user in opt_out_users:
                f.write(str(user) + '\n')
        await ctx.respond('This bot will not track your message content from now on. Most commands will no longer respond.', ephemeral=True)
...
async def opt_in(ctx: discord.ApplicationContext):
    ...
    if ctx.author.id in opt_out_users:
        ...
        with open(BASE_DIR / 'data' / 'opt-out-users.txt', 'w') as f:
            for user in opt_out_users:
                f.write(str(user) + '\n')
        await ctx.respond('This bot will now track the content of your messages. It will only be used to provide commands. Use the /privacy-policy command to view the privacy policy.', ephemeral=True)
    else:
        await ctx.respond('This bot is already tracking your message content.', ephemeral=True)
